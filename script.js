...
const start_bt = document.querySelector(".start_btn");
...
function showResultBox() {
    ...
    start_bt.style.opacity = "0";
    const scoreText = result_box.querySelector(".score_text");

    var marksCanvas = document.getElementById("marksChart");

    var marksData = {
    labels: ["HTML", "CSS", "Javascript", "PHP", "Node JS"],
    datasets: [{
        label: "score",
        backgroundColor: "rgba(0,200,132,0.1)",
        borderColor: 'rgb(0 , 200, 132)',
        data: [ htmlScore, cssScore, jsScore, phpScore, nodeScore]
...
function optionSelected(answer) {
    ...
    if (userAns == correctAns) {
        userScore += 1;
        if(questions[que_count].topic == "HTML" ) {
            htmlScore += 25;
        }
        if(questions[que_count].topic == "CSS" ) {
            cssScore += 25;
        }
        if(questions[que_count].topic == "javascript" ) {
            jsScore += 25;
        }
        if(questions[que_count].topic == "php" ) {
            phpScore += 25;
        }
        if(questions[que_count].topic == "Node" ) {
            nodeScore += 25;
        }
